...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Battlefield/Battlefield%20V.zip" -File "$env:SystemRoot\Temp\Battlefield V.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\Battlefield V.zip" -DestinationPath "$env:SystemRoot\Temp\Battlefield V" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# install config files
Copy-Item -Path "$env:SystemRoot\Temp\Battlefield V\*" -Destination "$env:USERPROFILE\Documents\Battlefield V\settings" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Copy-Item -Path "$env:SystemRoot\Temp\Battlefield V\*" -Destination "$env:USERPROFILE\OneDrive\Documents\Battlefield V\settings" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host
...
# install config file
Copy-Item "$env:SystemRoot\Temp\Battlefield V\user.cfg" -Destination "$InstallFolder" -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\Battlefield V" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Battlefield V.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
