...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Battlefield/Battlefield%203.zip" -File "$env:SystemRoot\Temp\Battlefield 3.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\Battlefield 3.zip" -DestinationPath "$env:SystemRoot\Temp\Battlefield 3" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# install config files
Copy-Item -Path "$env:SystemRoot\Temp\Battlefield 3\*" -Destination "$env:USERPROFILE\Documents\Battlefield 3\settings" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Copy-Item -Path "$env:SystemRoot\Temp\Battlefield 3\*" -Destination "$env:USERPROFILE\OneDrive\Documents\Battlefield 3\settings" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host
...
# install config file
Copy-Item "$env:SystemRoot\Temp\Battlefield 3\user.cfg" -Destination "$InstallFolder" -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\Battlefield 3" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Battlefield 3.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
