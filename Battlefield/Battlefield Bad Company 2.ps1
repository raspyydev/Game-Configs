...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Battlefield/Battlefield%20Bad%20Company%202.zip" -File "$env:SystemRoot\Temp\Battlefield Bad Company 2.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\Battlefield Bad Company 2.zip" -DestinationPath "$env:SystemRoot\Temp\Battlefield Bad Company 2" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# install config files
Copy-Item -Path "$env:SystemRoot\Temp\Battlefield Bad Company 2\*" -Destination "$env:USERPROFILE\Documents\BFBC2" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Copy-Item -Path "$env:SystemRoot\Temp\Battlefield Bad Company 2\*" -Destination "$env:USERPROFILE\OneDrive\Documents\BFBC2" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host
...
# cleanup
Remove-Item "$env:SystemRoot\Temp\Battlefield Bad Company 2" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Battlefield Bad Company 2.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
