...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Battlefield/Battlefield%206.zip" -File "$env:SystemRoot\Temp\Battlefield6.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\Battlefield6.zip" -DestinationPath "$env:SystemRoot\Temp\Battlefield6" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# download inspector
Get-FileFromWeb -URL "https://github.com/raspyydev/Better-Ressourcen/releases/download/downloads/inspector.exe" -File "$env:SystemRoot\Temp\Battlefield6\RebarOnInspector\inspector.exe"
Clear-Host
...
# install config file
Copy-Item "$env:SystemRoot\Temp\Battlefield6\user.cfg" -Destination "$InstallFolder" -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host
...
Start-Process -wait "$env:SystemRoot\Temp\Battlefield6\RebarOnInspector\inspector.exe" -args "$env:SystemRoot\Temp\Battlefield6\RebarOnInspector\Battlefield6.nip -silent"
Timeout /T 3 | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\Battlefield6" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Battlefield6.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host
