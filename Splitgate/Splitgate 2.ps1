...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Splitgate/Splitgate%202.zip" -File "$env:SystemRoot\Temp\Splitgate 2.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\Splitgate 2.zip" -DestinationPath "$env:SystemRoot\Temp\Splitgate 2" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# install config files
Copy-Item -Path "$env:SystemRoot\Temp\Splitgate 2\GameUserSettings.ini" -Destination "$env:LOCALAPPDATA\PortalWars2\Saved\Config\WindowsClient" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\Splitgate 2" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Splitgate 2.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host
