...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Splitgate/Splitgate%201.zip" -File "$env:SystemRoot\Temp\Splitgate 1.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\Splitgate 1.zip" -DestinationPath "$env:SystemRoot\Temp\Splitgate 1" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# install config files
Copy-Item -Path "$env:SystemRoot\Temp\Splitgate 1\GameUserSettings.ini" -Destination "$env:LOCALAPPDATA\PortalWars\Saved\Config\WindowsNoEditor" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\Splitgate 1" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Splitgate 1.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host
