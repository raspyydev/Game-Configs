...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Call%20of%20Duty/Call%20of%20Duty%20Black%20Ops%20Cold%20War.zip" -File "$env:SystemRoot\Temp\ColdWar.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\ColdWar.zip" -DestinationPath "$env:SystemRoot\Temp\ColdWar" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# install config files
Copy-Item -Path "$env:SystemRoot\Temp\ColdWar\player\*" -Destination "$env:USERPROFILE\Documents\Call Of Duty Black Ops Cold War\player" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Copy-Item -Path "$env:SystemRoot\Temp\ColdWar\player\*" -Destination "$env:USERPROFILE\OneDrive\Documents\Call Of Duty Black Ops Cold War\player" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host
...
# install config files
Copy-Item -Path "$env:SystemRoot\Temp\ColdWar\YourID\*" -Destination "$ConfigFolder1" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\ColdWar" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\ColdWar.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
