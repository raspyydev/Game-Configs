...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Call%20of%20Duty/Call%20of%20Duty%20Modern%20Warfare%202%202022.zip" -File "$env:SystemRoot\Temp\MW2.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\MW2.zip" -DestinationPath "$env:SystemRoot\Temp\MW2" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# edit config files
$options3cod22cst = "$env:SystemRoot\Temp\MW2\players\options.3.cod22.cst"
...
$content = Get-Content -Path "$env:SystemRoot\Temp\MW2\players\options.3.cod22.cst" -Raw
$filePath = "$env:SystemRoot\Temp\MW2\players\options.3.cod22.cst"
...
Copy-Item -Path "$env:SystemRoot\Temp\MW2\players\*" -Destination "$env:USERPROFILE\Documents\Call of Duty MWII\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Copy-Item -Path "$env:SystemRoot\Temp\MW2\players\*" -Destination "$env:USERPROFILE\OneDrive\Documents\Call of Duty MWII\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\MW2" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\MW2.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
