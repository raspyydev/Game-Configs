...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Call%20of%20Duty/Call%20of%20Duty%20Black%20Ops%206.zip" -File "$env:SystemRoot\Temp\BO6.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\BO6.zip" -DestinationPath "$env:SystemRoot\Temp\BO6" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# edit config files
$s10cod24txt0 = "$env:SystemRoot\Temp\BO6\players\s.1.0.cod24.txt0"
$s10cod24txt1 = "$env:SystemRoot\Temp\BO6\players\s.1.0.cod24.txt1"
...
$content = Get-Content -Path "$env:SystemRoot\Temp\BO6\players\s.1.0.cod24.txt0" -Raw
$filePath = "$env:SystemRoot\Temp\BO6\players\s.1.0.cod24.txt0"
...
$content = Get-Content -Path "$env:SystemRoot\Temp\BO6\players\s.1.0.cod24.txt1" -Raw
$filePath = "$env:SystemRoot\Temp\BO6\players\s.1.0.cod24.txt1"
$encoding = New-Object System.Text.UTF8Encoding $false
$writer = [System.IO.StreamWriter]::new($filePath, $false, $encoding)
$writer.Write($content)
$writer.Close()
...
Copy-Item -Path "$env:SystemRoot\Temp\BO6\players\*" -Destination "$env:USERPROFILE\Documents\Call of Duty\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Copy-Item -Path "$env:SystemRoot\Temp\BO6\players\*" -Destination "$env:USERPROFILE\OneDrive\Documents\Call of Duty\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\BO6" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\BO6.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
