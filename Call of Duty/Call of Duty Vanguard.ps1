...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Call%20of%20Duty/Call%20of%20Duty%20Vanguard.zip" -File "$env:SystemRoot\Temp\Vanguard.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\Vanguard.zip" -DestinationPath "$env:SystemRoot\Temp\Vanguard" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# edit config files
$optionsvanguard = "$env:SystemRoot\Temp\Vanguard\players\options.Vanguard"
...
$content = Get-Content -Path "$env:SystemRoot\Temp\Vanguard\players\options.Vanguard" -Raw
$filePath = "$env:SystemRoot\Temp\Vanguard\players\options.Vanguard"
...
Copy-Item -Path "$env:SystemRoot\Temp\Vanguard\players\*" -Destination "$env:USERPROFILE\Documents\Call of Duty Vanguard\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Copy-Item -Path "$env:SystemRoot\Temp\Vanguard\players\*" -Destination "$env:USERPROFILE\OneDrive\Documents\Call of Duty Vanguard\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\Vanguard" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Vanguard.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
