...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Call%20of%20Duty/Call%20of%20Duty%20Modern%20Warfare%202019.zip" -File "$env:SystemRoot\Temp\2019.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\2019.zip" -DestinationPath "$env:SystemRoot\Temp\2019" -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# edit config files
$advoptionsini = "$env:SystemRoot\Temp\2019\players\adv_options.ini"
...
$content = Get-Content -Path "$env:SystemRoot\Temp\2019\players\adv_options.ini" -Raw
$filePath = "$env:SystemRoot\Temp\2019\players\adv_options.ini"
...
Copy-Item -Path "$env:SystemRoot\Temp\2019\players\*" -Destination "$env:USERPROFILE\Documents\Call of Duty Modern Warfare\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Copy-Item -Path "$env:SystemRoot\Temp\2019\players\*" -Destination "$env:USERPROFILE\OneDrive\Documents\Call of Duty Modern Warfare\players" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\2019" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\2019.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
