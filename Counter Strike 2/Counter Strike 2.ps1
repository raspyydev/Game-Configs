...
# Config archive: prefer zip next to script, else download; require zip before folder picker.
# Embedded BWO / dot-source: resolve zip path like BO7 (no Join-Path on empty bundle root).
$cs2ZipName = 'Counter Strike 2.zip'
$tempZip = Join-Path $env:SystemRoot 'Temp\Counter Strike 2.zip'
$tempDir = Join-Path $env:SystemRoot 'Temp\Counter Strike 2'
...
if ([string]::IsNullOrWhiteSpace("$scriptDir") -and -not [string]::IsNullOrWhiteSpace($env:BWO_GAME_CONFIGS_ROOT)) {
    ...
    if ($gr.Length -gt 0) {
        $scriptDir = [System.IO.Path]::Combine($gr, 'Counter Strike 2')
    }
}
...
if ($_sdFinal.Length -gt 0 -and -not [string]::IsNullOrWhiteSpace($cs2ZipName)) {
    $localZip = [System.IO.Path]::Combine($_sdFinal, $cs2ZipName)
}
...
try {
    if ($localZip -and (Test-Path -LiteralPath $localZip)) {
        ...
        Get-FileFromWeb -URL 'https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Counter%20Strike%202/Counter%20Strike%202.zip' -File $tempZip
    }
} catch {
    Write-Host "Counter Strike 2 download failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
Clear-Host

if (-not (Test-Path -LiteralPath $tempZip) -or ((Get-Item -LiteralPath $tempZip).Length -lt 64)) {
    Write-Host 'Counter Strike 2 config archive missing or empty.' -ForegroundColor Red
    exit 1
}
...
try {
    ...
    Write-Host "Counter Strike 2 extract failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}

$extracted = @(Get-ChildItem -LiteralPath $tempDir -Force -ErrorAction SilentlyContinue)
if ($extracted.Count -eq 0) {
    Write-Host 'Counter Strike 2 archive expanded to an empty folder.' -ForegroundColor Red
    exit 1
}
Clear-Host

# pick folder
Write-Host "In 'C:\Program Files (x86)\Steam\userdata\YourID\730\local' select 'cfg' folder:"
$ConfigFolder = Show-ModernFilePicker -Mode Folder
if ([string]::IsNullOrWhiteSpace($ConfigFolder)) {
    Write-Host 'No cfg folder selected. Cancelled or dialog failed.' -ForegroundColor Red
    exit 1
}

# install config files
try {
    Copy-Item -Path (Join-Path $tempDir '*') -Destination $ConfigFolder -Recurse -Force
} catch {
    Write-Host "Counter Strike 2 copy failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\Counter Strike 2" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Counter Strike 2.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
if (-not [string]::IsNullOrWhiteSpace($ConfigFolder)) {
    Remove-Item (Join-Path $ConfigFolder '-allow_third_party_software.launchoption') -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
}
Clear-Host

# message
Write-Host "Counter Strike 2 config applied . . ."
Write-Host ""
Write-Host "Disable FidelityFX Super Resolution"
Write-Host ""
Write-Host ""
Write-Host "----------------------------------------------------"
Write-Host "POWER USERS & HIGH END MACHINES ONLY + TEST YOURSELF"
Write-Host "----------------------------------------------------"
Write-Host "For better 1% lows try"
Write-Host ""
Write-Host "- HAGS off"
Write-Host "- REBAR off"
Write-Host "- Reflex off"
Write-Host "- FSE (legacy flip)"
Write-Host "- High prio (launch option wont work)"
Write-Host "- Core 1 & Thread 1 Off (affinity not bios)"
