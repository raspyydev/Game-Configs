...
Get-FileFromWeb -URL "https://github.com/raspyydev/Game-Configs/raw/refs/heads/main/Delta%20Force/Delta%20Force.zip" -File "$env:SystemRoot\Temp\Delta Force.zip"
Clear-Host

# extract config files
Expand-Archive "$env:SystemRoot\Temp\Delta Force.zip" -DestinationPath "$env:SystemRoot\Temp\Delta Force" -ErrorAction SilentlyContinue | Out-Null
Clear-Host
...
Copy-Item -Path "$env:SystemRoot\Temp\Delta Force\*" -Destination "$ConfigFolder" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Clear-Host

# cleanup
Remove-Item "$env:SystemRoot\Temp\Delta Force" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
Remove-Item "$env:SystemRoot\Temp\Delta Force.zip" -Recurse -Force -ErrorAction SilentlyContinue | Out-Null
